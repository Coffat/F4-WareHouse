...
describe('04. Operations - Nhập Xuất Kho', () => {
  ...
  it('TC21 - Mở form tạo phiếu nhập kho', () => {
    cy.visit('/operations');
    cy.contains(/inbound|nhập kho/i).click({ force: true });
    
    // Click button tạo phiếu nhập
    cy.contains('button', /tạo|thêm|new|add|nhập mới/i).first().click({ force: true });
    
    // Form hoặc modal phải xuất hiện
    cy.get('[role="dialog"], .modal, form').should('exist');
