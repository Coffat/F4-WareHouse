...
describe('01. Authentication', () => {
  ...
  it('TC02 - Đăng nhập thất bại với sai credentials → hiển thị lỗi', () => {
    ...
    // Chờ thông báo lỗi xuất hiện (toast hoặc error text)
    cy.get('body').should('contain.text', '').and(() => {
      // Vẫn ở trang login
      expect(window.location.pathname).to.include('/auth/login');
    });
    cy.url().should('include', '/auth/login');
    cy.log('✅ Login thất bại hiển thị lỗi đúng');
  });
